...
function buildHelpContent() {
  ...
  return `
${chalk.bold.white('                  Okalit CLI')} ${chalk.bgHex('#8b5cf6').white(' v0.5.5 ')}
...
async function handleAdd(argv, cwd) {
  ...
  // Separate remote dependencies (URLs pointing to other repos)
  const remoteDeps = extractRemoteDependencies(registryData);

  spinner.text = chalk.blue(`Installing component(s)...`);

  const installed = [];
  const skipped = [];

  // Install remote dependencies first
  for (const remoteUrl of remoteDeps) {
    const remoteRegistry = remoteUrl;
    try {
      const remoteData = await fetchRegistryJson(remoteRegistry);
      const remoteName = remoteData.name || remoteData.tag || path.basename(remoteUrl);
      const normalizedRemote = normalizeRegistryData(remoteData, remoteName);
      const remoteEntry = normalizedRemote.components?.[remoteName];
      if (!remoteEntry) continue;

      const remoteDest = resolveDestination(projectRoot, null, remoteEntry.type || 'component', remoteName);
      if (fs.existsSync(remoteDest)) {
        skipped.push(remoteName);
        continue;
      }

      const files = remoteEntry.files || [];
      const remotePath = remoteEntry.path || `components/${remoteName}`;
      fs.mkdirSync(remoteDest, { recursive: true });

      for (const file of files) {
        const content = await fetchFileFromRegistry(remoteRegistry, remotePath, file);
        fs.writeFileSync(path.join(remoteDest, file), content, 'utf8');
      }

      installed.push({ name: remoteName, dest: remoteDest, type: remoteEntry.type || 'component', registry: remoteRegistry });
    } catch (err) {
      console.log(chalk.yellow(`  ⚠ Remote dep "${remoteUrl}": ${err.message}`));
    }
  }
  ...
  for (const depName of installPlan) {
    ...
    installed.push({ name: depName, dest: depDest, type: depType, registry });
  }

  // Track installed components in lock file
  if (installed.length) {
    const lock = readLockFile(projectRoot);
    for (const { name, dest, type, registry: depRegistry } of installed) {
      lock[name] = {
 ...
 * Skips remote dependencies (URLs) — those are handled separately.
 */
...
/**
 * Extracts remote dependencies (URLs) from all components in the registry.
 */
function extractRemoteDependencies(registryData) {
  const remotes = new Set();
  for (const entry of Object.values(registryData.components || {})) {
    for (const dep of entry.dependencies || []) {
      if (isRemoteDep(dep)) remotes.add(dep);
    }
  }
  return [...remotes];
}

function isRemoteDep(dep) {
  return dep.includes('/');
}
