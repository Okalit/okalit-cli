...
function buildHelpContent() {
  ...
  return `
${chalk.bold.white('                  Okalit CLI')} ${chalk.bgHex('#8b5cf6').white(' v0.5.2')}
...
async function handleUpdate(argv, cwd) {
  const targetName = argv[0]; // optional: update specific component
  const projectRoot = findProjectRoot(cwd);
  const lock = readLockFile(projectRoot);

  if (Object.keys(lock).length === 0) {
    console.log(chalk.yellow('No catalog components installed. Use `okalit add` first.'));
    return;
  }
  ...
  const spinner = ora({
    text: chalk.blue(`Updating ${Object.keys(toUpdate).length} component(s)...`),
    spinner: 'arc',
    color: 'magenta',
  }).start();
  ...
  for (const [name, meta] of Object.entries(toUpdate)) {
    try {
      ...
      // Re-download all files (overwrite)
  ...
  spinner.succeed(gradient(['#10b981', '#06b6d4'])('Update complete!'));
